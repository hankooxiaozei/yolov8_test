...
def interactive_check_labels(txt_labels_dir, images_dir):
    ...
    while True:
        ...
        processed_img = preprocess_pipeline(original_img.copy(), d, sigma_color, sigma_space, clip_limit, tile_size,
                                            use_sharpen)
        ...
        try:
            ...
            for new_info in new_cnt_info:
                ...
                cv2.polylines(processed_img, [np.array(points, np.int32)], True, color_map.get(class_id, (0, 0, 255)),
                              2)
        ...
        info_text = (f"File: {display_filename} | d={d}, sC={sigma_color}, sS={sigma_space}, "
        ...
        # 使用定义的变量来绘制带描边的文本
        cv2.putText(combined_display, info_text, text_position, font_face, font_scale,
                    outline_color, outline_thickness, cv2.LINE_AA)
        cv2.putText(combined_display, info_text, text_position, font_face, font_scale,
                    font_color, text_thickness, cv2.LINE_AA)
        # ================================================================
